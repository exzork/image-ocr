...
def test_pytorch_identical_output():
    ...
    model_keras = image_ocr.detection.build_keras_model(weights_path=weights_path_keras)
    model_pytorch = image_ocr.detection.build_torch_model(
        weights_path=weights_path_torch
    )
    image = image_ocr.tools.read("tests/test_image.jpg")
    X = image_ocr.detection.compute_input(image)[np.newaxis,]
